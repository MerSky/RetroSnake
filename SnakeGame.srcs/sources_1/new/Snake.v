...
(
	...
	reg [5:0]cube_x[15:0];
	reg [5:0]cube_y[15:0];//体长坐标 单位："格子" ，限长16
	reg [15:0]is_exist;//用于控制身子每个格子是否显示，进而控制身子长度
	...
	always @(posedge clk or negedge rst) begin
		if(!rst) begin
			...
			cnt <= cnt + 1;
			if(cnt == 12_500_000) begin //0.02us*12500000=0.25s，每0.25s移动一次
				cnt <= 0;
	...
	//若吃到苹果则add_cube==1，显示体长增加一位，"is_exixt[cube_num]<=1",让第cube_num位显示出来
	always @(posedge clk or negedge rst) begin
		if(!rst) begin
		//初始显示长度为3，is_exist=0000 0000 0000 0111,表示前三位出现
			is_exist <= 16'd7;
			cube_num <= 3;
			addcube_state <= 0;
		end
